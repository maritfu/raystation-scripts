...
class QualityControl(object):
  def __init__(self, patient, case, plan):
    ...
    #ts_case.localization_points_for_gating_test() tas bort (sjekk om poi_geometry test virker)
    ...
    for ts_structure_set in ts_case.ts_structure_sets:
      if ts_structure_set.structure_set.OnExamination.Name == ts_case.ts_plan.plan.GetStructureSet().OnExamination.Name:
        ts_structure_set.body_for_free_breething_ct() #Må testes
        ts_structure_set.localization_point_test()
        ts_structure_set.external_test()
        #ts_structure_set.dose_region_test() #tas bort
        ts_structure_set.ptv_derived_test()
        ts_structure_set.prosthesis_titanium_test()
        ts_structure_set.external_ptv_bounding_test()
        #ts_structure_set.couch_test() må tas bort eller endres
        #ts_structure_set.couch_close_to_patient_test() tas bort 
        ts_structure_set.breast_seeds_test() #tas kanskje bort
        ts_structure_set.density_override_test()
      
        # POI geometry tests:
        for ts_poi_geometry in ts_structure_set.ts_poi_geometries:
          ts_poi_geometry.is_defined_test() #test om virker rett for mamma DIBH
    ...
    ts_plan.prescription_mu_breast_regional_caudal_test()
    ts_plan.prescription_mu_breast_regional_cranial_test()
    ts_plan.beam_number_for_breast_test()
    ts_plan.equal_isocenter_for_beams_test()
    ts_plan.prescription_mu_breast_test()
    ...
    for ts_beam_set in ts_plan.ts_beam_sets:
      ...
      for ts_beam in ts_beam_set.ts_beams:
        ...
        ts_beam.max_mu_for_sliding_window_test()
