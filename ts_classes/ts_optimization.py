...
class TSOptimization(object):
  ...
  def is_sliding_window(self):
    ...
    if arc_properties:
      if arc_properties.UseSlidingWindowSequencing:
        match = True
      if match:
        return True
      else:
  ...
  # Tests if Constrain leaf motion of 0.5 cm/deg is used, or 2 for sliding window MÅ TESTES
  def constrain_leaf_motion_test(self):
    t = TEST.Test("Skal i utgangspunktet bruke Constrain leaf motion = 0.5 cm/deg, og 2 cm/deg for sliding window", True, self.parameter)
    match = True
    t.expected = 0.5
    ...
    if arc_properties:
      if arc_properties.UseMaxLeafTravelDistancePerDegree:
        if self.is_sliding_window():
          t.expected = 2
        if arc_properties.MaxLeafTravelDistancePerDegree != t.expected:
          match = False
      else:
        match = False
      if match:
        ...
        return t.fail(arc_properties.MaxLeafTravelDistancePerDegree)
  ...
  def dose_grid_test(self):
    correct_grid = 0.3
    examination = self.ts_beam_set.beam_set.GetPlanningExamination()
    sp = examination.Series[0].ImageStack.SlicePositions
    ct_grid = sp[1]-sp[0]
    grid_x = grid_y = grid_z = ct_grid
    grid = self.optimization.OptimizationParameters.TreatmentSetupSettings[0].ForTreatmentSetup.FractionDose.InDoseGrid.VoxelSize
    match = True
    #if self.ts_beam_set.ts_label.label.region in RC.prostate_codes:
    #  grid_x = grid_y = 0.3
    if round(grid.x,1) != round(grid_x,1) or round(grid.y,1) != round(grid_y,1) or round(grid.z,1) != round(grid_z,1):
      match = False
    t = TEST.Test("Skal i utgangspunktet bruke dosegrid: " + str(grid_x)+"x"+str(grid_z)+"x"+str(grid_y)+" cm^3", True, self.grid)
    if match:
      return t.succeed()
    else:
      return t.fail(grid.x)
