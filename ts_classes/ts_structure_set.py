...
class TSStructureSet(object):
  ...
  def body_for_free_breething_ct(self):
    t = TEST.Test("Struktursettet må ha definert en 'xBody_fripust', og den må være satt som 'Control'", True, self.geometry)
    match = True
    ...
    if self.ts_case.ts_plan.ts_beam_sets[0].ts_label.label.region:
      if self.ts_case.ts_plan.ts_beam_sets[0].ts_label.label.region in RC.breast_codes:
        match = False
        if self.ts_case.ts_plan.ts_beam_sets[0].ts_label.label.region in RC.breast_l_codes:
          for rg in self.structure_set.RoiGeometries:
            if rg.OfRoi.Name == ROIS.x_body_fripust.name and rg.OfRoi.Type == 'Control' and rg.HasContours():
              match = True
        else:
          match = True
  ...
  # Tests if the CT-images are cropped close to body.
  def external_bounding_test(self):
    ...
    for rg in self.structure_set.RoiGeometries:
      if SSF.has_named_roi_with_contours(self.structure_set, ROIS.body.name):
  ...
  # Tests if the CT-images are cropped close to body, in the same area as there is a target volume.
  def external_ptv_bounding_test(self):
    t = TEST.Test("External ligger helt på yttergrensen av bildeopptaket og PTV er i nærheten. Dette kan tyde på at CT-bildene er beskjært uheldig, og at deler av pasienten mangler. Dersom denne beskjæringen forekommer i nærheten av målvolumet, vil det resultere i feil doseberegning.", True, self.external_bounding)
    # Run test if this structure set corresponds to the examination used for the treatment plan:
    #t.expected = 0
    match = False
    for rg in self.structure_set.RoiGeometries:
      if SSF.has_named_roi_with_contours(self.structure_set, ROIS.body.name):
        ext_box = self.structure_set.RoiGeometries[ROIS.body.name].GetBoundingBox()
  ...
  def density_override_test(self):
    t = TEST.Test("Det kan se ut som et tetthetsvolum er på. Vanligvis skal tetthetsvolum slåes av ved godkjenning.", True, self.geometry)
    match = True
    for rg in self.structure_set.RoiGeometries:
      if rg.OfRoi.RoiMaterial:
        if rg.OfRoi.RoiMaterial.OfMaterial.Name == 'Water' and rg.OfRoi.RoiMaterial.OfMaterial.MassDensity == 1: 
          match = False
