...
def setup_clinical_goals(ss, es, site, total_dose, nr_fractions, target):
  for cg in site.target_clinical_goals:
    ...
    if SSF.has_roi(ss, cg.name):
      #messagebox.showinfo("", str(cg.tolerance))
      ...
      elif cg.type in [volume_at_dose,abs_volume_at_dose] and cg.tolerance >2:
        c = es.AddClinicalGoal(RoiName = cg.name, GoalCriteria = cg.criteria, GoalType = cg.type, AcceptanceLevel =cg.value , ParameterValue = round(cg.tolerance*100,0), Priority = cg.priority)
      elif cg.type in[volume_at_dose,abs_volume_at_dose] and cg.tolerance <2:
        #messagebox.showinfo("", str(cg.tolerance))
        c = es.AddClinicalGoal(RoiName = cg.name, GoalCriteria = cg.criteria, GoalType = cg.type, AcceptanceLevel =cg.value , ParameterValue = round(cg.tolerance*total_dose*100,0), Priority = cg.priority)
      elif cg.tolerance >2:
        c = es.AddClinicalGoal(RoiName = cg.name, GoalCriteria = cg.criteria, GoalType = cg.type, AcceptanceLevel = round(cg.tolerance*100,0), ParameterValue = cg.value, Priority = cg.priority)
      elif cg.type in [dose_at_abs_volume, dose_at_volume] and cg.tolerance <2:
        #messagebox.showinfo("", str(cg.tolerance))
        c = es.AddClinicalGoal(RoiName = cg.name, GoalCriteria = cg.criteria, GoalType = cg.type, AcceptanceLevel = round(cg.tolerance*total_dose*100,0), ParameterValue = cg.value, Priority = cg.priority)
    else:
      # Missing ROI:
