# Handling the Trondheim beam set label format:
class BeamSetLabel(object):
  ...
  def process_technique(self):
    if not self.parts[0][0].isalpha():
      self.valid = False
    else:
      self.technique = self.parts[0][0]

  # First characters of first section should be integer(s):
  def process_region(self):
    if not self.parts[0][1:].isdigit():
      self.valid = False
    else:
      self.region = int(self.parts[0][1:])
