...
class DefSite(object):
  ...
  def __init__(self, patient_db, pm, examination, ss, choices, targets, oars):
    ...
    PMF.create_external_body_and_couch_geometry(self.pm, self.examination, self.ss, couch_thickness) 
    #  if self.choices[1] == 'stereotactic':
        # Create external geometry:
        #PMF.create_external_body_and_couch_geometry(self.pm, self.examination, self.ss) BRUK DENNE 
