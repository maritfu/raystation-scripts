...
def check_input(ss, region_code, nr_fractions, fraction_dose):
  if region_code in RC.brain_codes:
    ...
    error(nr_fractions, fraction_dose, [10, 25], [1.8, 2.2, 2.3, 3])
    ...
    elif SSF.has_roi_with_shape(ss, ROIS.ctv_p_54.name):
      error(nr_fractions, fraction_dose, [27], [2])
    elif SSF.has_roi_with_shape(ss, ROIS.ctv_p_57.name):
      error(nr_fractions, fraction_dose, [27], [2.13])
    else:
...
def collect_fractionation_choices(my_window):
  ...
  if frame.ok:
    ...
    total_dose = round(nr_fractions * fraction_dose,2)
  elif not frame.ok:
    print ("Script execution cancelled by user...")
    sys.exit(0)
  else:
