...
# Create optimization objects, maximum dose
def max_dose_robust(ss, plan, roi_name, dose_level, weigth, beam_set_index=0):
  if SSF.has_named_roi_with_contours(ss, roi_name):
    po = plan.PlanOptimizations[beam_set_index]
    o = po.AddOptimizationFunction(FunctionType="MaxDose", RoiName = roi_name, IsRobust = True)
    o.DoseFunctionParameters.DoseLevel = dose_level
    o.DoseFunctionParameters.Weight = weigth
    return o
  else:
...
# Create optimization objects, minimum dose
def min_dose_robust(ss, plan, roi_name, dose_level, weigth, beam_set_index=0):
  if SSF.has_named_roi_with_contours(ss, roi_name):
    po = plan.PlanOptimizations[beam_set_index]
    o = po.AddOptimizationFunction(FunctionType="MinDose", RoiName = roi_name, IsRobust = True)
    o.DoseFunctionParameters.DoseLevel = dose_level
    o.DoseFunctionParameters.Weight = weigth
    return o
  else:
...
def set_robustness_non_planning_examinations(plan, new_examination_names):
  plan.PlanOptimizations[0].OptimizationParameters.SaveRobustnessParameters(
    ...
    IndependentSuperiorInferior = True,
    ComputeExactScenarioDoses=False,
    NamesOfNonPlanningExaminations=new_examination_names
  )
